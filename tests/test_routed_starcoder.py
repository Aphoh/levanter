...
def test_routed_starcoder_forward():
    Batch = hax.Axis("batch", 32)
    with get_mesh(Batch):
        config = RStarcoderConfig(seq_len=512, num_layers=2, hidden_dim=256, intermediate_dim=512)
        Vocab = hax.Axis("vocab", 1000)
        key = jax.random.PRNGKey(0)
        model = RStarcoderLMHeadModel.init(Vocab, config, key=key)
...
@skip_if_no_torch
@pytest.mark.parametrize("expert_type", [ExpertType.LORA, ExpertType.MLP])
@pytest.mark.parametrize("expert_init", [t for t in ExpertInit])
def test_rstarcoder_consistent_with_base_starcoder(expert_type, expert_init):
    import torch
    ...
    converter = RStarcoderConfig().hf_checkpoint_converter()
    ...
    torch_model = Starcoder2ForCausalLM(hf_config)
    torch_model.eval()
    ...
    with get_mesh(Batch):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            model: RStarcoderLMHeadModel = converter.load_pretrained(
            ...
            embeds = model.embeddings.embed(example.tokens)
            torch_embeds = torch_model.model.embed_tokens(input_torch)
            assert np.isclose(embeds.array, torch_embeds.detach().cpu().numpy(), rtol=1e-2, atol=1e-4).all()

            @hax.named_jit
            def compute(model: RStarcoderLMHeadModel, example: RoutableLmExample):
...
def init_const_model(
...
) -> tuple[RStarcoderLMHeadModel, RStarcoderConfig]:
    config = RStarcoderConfig(
        seq_len=512,
        num_layers=2,
        hidden_dim=256,
    ...
    model: RStarcoderLMHeadModel = RStarcoderLMHeadModel.init(
        hax.Axis("vocab", 100), config, key=jax.random.PRNGKey(0)
    )
...
@pytest.mark.parametrize("expert_type", [ExpertType.LORA, ExpertType.MLP])
def test_rstarcoder_trainable_filt(expert_type):
    config = RStarcoderConfig(
        seq_len=512,
        num_layers=2,
        hidden_dim=256,
    ...
    is_trainable: RStarcoderLMHeadModel = routed_experts_trainable_params_filter(model_shape)
...
def test_rstarcoder_expert_type_init():
    config = RStarcoderConfig(
        seq_len=512,
        num_layers=2,
        hidden_dim=256,
...
def test_weight_masks():
    ...
    experts_mask: RStarcoderLMHeadModel = routed_experts_mask(model_shape)

    def manual_expert_values(mask: RStarcoderLMHeadModel):
    ...
    base_mask: RStarcoderLMHeadModel = base_weights_mask(model_shape)
